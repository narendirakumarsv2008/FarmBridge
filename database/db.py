...
# Useful indexes. Each entry is (name, table, columns). The loader skips an
# index that already exists, so these are safe to run on every startup.
#
# Why each index exists:
# - listings(status, created_at): marketplace filters active listings and sorts by newest.
# - listings(farmer_id): farmer dashboard / farmer order lookups.
# - listings(crop_name): marketplace search/category filtering.
# - orders(buyer_user_id, created_at): consumer "my orders" list.
# - orders(status): farmer order-status filtering and monitoring.
# - order_items(order_id): order detail / farmer order assembly.
# - order_items(listing_id): listing-level order history / stock reconciliation.
# - order_items(farmer_id): farmer order inbox.
# - subscriptions(buyer_phone, active): consumer active plans / delivery calendar.
# - pool_joins(pool_id): pool volume aggregation.
INDEXES = {
    'listings': [
        ('idx_listings_status_created', ['status', 'created_at']),
        ('idx_listings_farmer', ['farmer_id']),
        ('idx_listings_crop', ['crop_name']),
    ],
    'orders': [
        ('idx_orders_consumer_created', ['buyer_user_id', 'created_at']),
        ('idx_orders_status', ['status']),
    ],
    'order_items': [
        ('idx_order_items_order', ['order_id']),
        ('idx_order_items_listing', ['listing_id']),
        ('idx_order_items_farmer', ['farmer_id']),
    ],
    'subscriptions': [
        ('idx_subs_consumer_active', ['buyer_phone', 'active']),
    ],
    'pool_joins': [
        ('idx_pool_joins_pool', ['pool_id']),
    ],
}
...
def _ssl_params():
    """Map MYSQL_SSL_MODE to PyMySQL ssl parameters.

    - disabled: no SSL parameters.
    - preferred / require: encrypted connection without cert verification
      (good default for most managed MySQL providers).
    - verify-ca / verify-identity: require verified SSL (CA/identity).
    """
    mode = _CFG.MYSQL_SSL_MODE
    if mode in ('disabled', 'none', 'off', 'false'):
        return None
    if mode in ('verify-ca', 'verify-identity'):
        ca = os.environ.get('MYSQL_SSL_CA', '')
        return {'ca': ca or None, 'check_hostname': mode == 'verify-identity'}
    # preferred / require
...
def _mysql_conn(db=None):
    ...
    cfg = dict(
        ...
        charset='utf8mb4',
        autocommit=False,
        connect_timeout=_CFG.MYSQL_CONNECT_TIMEOUT,
    )
    if db is not None and not db:
        cfg.pop('database', None)
    ssl_params = _ssl_params()
    if ssl_params:
        cfg['ssl'] = ssl_params
...
def _try_mysql(cfg):
    ...
    # Preferred path: connect directly to the target database. Most managed
    # cloud MySQL providers already create the database and do NOT grant the
    # application user CREATE DATABASE permission.
    try:
        conn = _mysql_conn()
        conn.close()
        return True
    except Exception as direct_err:
        first = str(direct_err).split('\n')[0][:160]

    # Only attempt to CREATE DATABASE when explicitly enabled (e.g. local
    # Docker MySQL) or when running in development.
    if not cfg.MYSQL_CREATE_DATABASE and not cfg.is_production:
        if cfg.allow_sqlite_fallback:
            print('[db] MySQL unavailable (%s)' % first)
            return False
        raise RuntimeError('Production requires a reachable MySQL database: %s' % first)
    ...
    try:
        ...
        if not cfg.allow_sqlite_fallback:
            raise RuntimeError(
                'Production requires a reachable MySQL database: %s. '
                'If the database already exists, set MYSQL_CREATE_DATABASE=false '
                'and make sure the app user can connect to it.' % msg
            )
...
def _existing_indexes(conn, table):
    """Return a set of existing index names for a table."""
    cur = conn.cursor()
    try:
        if conn.engine == 'sqlite':
...
def _apply_indexes(conn):
    """Idempotently create the useful indexes. Never drops or alters data."""
    for table, indexes in INDEXES.items():
        ...
        for name, cols in indexes:
            ...
            cur.execute(sql % (name, table, ', '.join(cols)))
    conn.commit()
...
def engine_info(cfg=None):
    """Internal diagnostics (never returned by public /api/db-info)."""
    c = cfg or _CFG
...
def get_db_status():
    """Safe health check. Returns connected True/False without secrets."""
    if not ENGINE:
    ...
    try:
        ...
        conn.close()
        return {'connected': connected, 'engine': ENGINE}
    except Exception:
        return {'connected': False, 'engine': ENGINE}
