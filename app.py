...
from database.db import get_db_status, init_db as init_database, engine_info
...
def _configure_cors(app):
    ...
    if config.is_production and '*' in origins:
        app.logger.warning(
            'CORS is set to "*" in production. For authenticated cross-origin '
            'APIs, set ALLOWED_ORIGINS to your frontend origin(s).'
        )
...
def create_app(test_config=None):
    ...
    if test_config:
        for k, v in test_config.items():
            if k == 'ENVIRONMENT':
                ...
                config.SECRET_KEY = v
            elif k == 'STORAGE_PROVIDER':
                config.STORAGE_PROVIDER = v
            elif k == 'UPLOAD_FOLDER':
                config.UPLOAD_FOLDER = v
            else:
                app.config[k] = v
    ...
    def health():
        ...
        return jsonify({
            ...
            'app': 'FarmBridge',
            'database': {'engine': db.get('engine'), 'connected': db.get('connected')},
    ...
    # Log startup state (no credentials, no full connection strings).
    app.logger.info(
        'FarmBridge starting in %s mode | DB engine=%s | storage=%s | debug=%s',
        config.ENVIRONMENT, engine_info().get('engine'),
        config.STORAGE_PROVIDER, app.debug,
    )
