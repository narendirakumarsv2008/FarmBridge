...
For the full beginner guide, see **`docs/RENDER_DEPLOYMENT_GUIDE.md`**.
...
3. Start command:
   ...
   ```
   Gunicorn reads Render's `PORT` automatically from `gunicorn.conf.py`.
4. Create a cloud MySQL database (Railway, Aiven, DigitalOcean, AWS RDS, etc.).
5. Add environment variables in the Render dashboard:
   ...
   ```
6. Deploy.
7. Visit `/health` and `/api/health` to confirm the database is connected.
